...
def write_data_to_sheet(data, line_number, sheet_name):
    try:
        ...
        def get_range_width(a1_range):
            # Support simple ranges like G12:Q12 from settings.env.
            ...
            return column_to_number(end_col) - column_to_number(start_col) + 1
        ...
        if data is None:
            note = "Not Found"
            # Overwrite the full destination range to clear any stale values.
            range_width = get_range_width(range_name)
            if range_width < 2:
                range_width = 2
            not_found_row = [note, updated_time] + [""] * (range_width - 2)
            worksheet.update(range_name=range_name, values=[not_found_row])
            return None
